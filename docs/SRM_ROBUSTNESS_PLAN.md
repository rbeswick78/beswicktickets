...
| 4 — Client responsiveness | ⬜ Not started | Depends on Phase 3 (absolute echoes + acks + rev are now in place). Optimistic chips, pointer events, reconnect resync. |
| 5 — Structural refactor | ⬜ Not started | Module split. Do last. |

**Phase 0 commit state:** merged and pushed to `main` (commit `ccc1b30` + doc updates). Phase 1 onward works directly on `main` per the workflow above. Phase 1 = `bcee21c`; Phase 2 = `bf82e3c`; Phase 3 committed and pushed to `main` (see git log).
