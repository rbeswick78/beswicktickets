...
| 5 — Structural refactor | ✅ Complete (2026-06-23) | Pure structural move, **zero behavior change**. Split the 1858-line `public/js/srmGameBoard.js` monolith into a thin entry + **10 ES modules under `public/js/srm/`**: `state.js` (the ONE shared-state object replacing the ~24 bare module-level globals, plus the `socket`/`reconciler` singletons + re-exported SrmBet helpers), `constants.js` (`TIMING`/`MOVE_CANCEL_PX`), `util.js` (`dbg`/`delay`/`getBetKey`/`makeBatchId`/`showToast`), `board.js` (`spotClass`/`getBetSpotElement`/`positionChips`/`getUserColor`/`setChipAmount`), `ledBalance.js`, `cards.js`, `audio.js` (`initAudio`), `bets.js` (chips + optimistic taps + the socket batch/ack + `rebuildUIFromState` — chip-create↔`placeBet` is recursive so they stay in one module), `results.js` (per-card reveal + summary), `celebrate.js` (longshot). The entry (`srmGameBoard.js`, 581 lines) keeps ONLY the module-eval `preloadAllCardImages()` + the `DOMContentLoaded` wiring (identity from `<body>` data-attrs, HUD/chip-selector/summary toggles, audio init, all 11 Socket.IO handlers + pointer/click listeners). **Import graph is a strict acyclic DAG** (state/constants/util are leaves; board/led/cards/audio → leaves; bets/results/celebrate → those; only the entry imports the feature modules). `window.gameId`/`window.currentUserId`/`window.isDealer` threaded through `state.*` (this file was their only consumer; nothing external reads `window`). `srmBetReconciler.js` kept **AS-IS** (classic `<script>` → `window.SrmBet`, still `require()`d by node:test); `gameBoard.ejs` unchanged (still loads the reconciler classic-script before the deferred module, so `window.SrmBet`/`window.io` are ready at `state.js` eval). Verified: `node --check` clean on all 12 files; `npm test` 91/95 unchanged (4 MONGODB_URI-gated); grep gate (no bare un-converted global, no leftover `window.*` identity, socket event names + payload keys byte-identical); a throwaway Node ESM harness (browser globals stubbed) imported the full graph AND fired all 11 socket handlers incl. the `cardsDealt` reveal with **zero ReferenceError** (proves every import resolves / no forgotten export / no eval-breaking cycle). **Not** run against the live app (no local MongoDB; no client test harness per §7 — rides the manual matrix). **PROGRAM COMPLETE — all phases 0–5 done; no further handoff prompt needed.** |

**Phase 0 commit state:** merged and pushed to `main` (commit `ccc1b30` + doc updates). Phase 1 onward works directly on `main` per the workflow above. Phase 1 = `bcee21c`; Phase 2 = `bf82e3c`; Phase 3 = `f62fcfd`; Phase 4 = `575c3ab`; Phase 5 committed and pushed to `main` (see git log).

**🏁 Program complete (2026-06-23).** All six phases (0–5) are merged on `main`. The reported betting bug — real bet loss near a deal, and the *felt* "taps don't register" — is fully resolved at the data layer (Phases 1–2), the protocol is self-healing (Phase 3), the felt is optimistic + reconnect-safe (Phase 4), and the client is now modular (Phase 5). There is **no further handoff prompt**; future work would be net-new features, not a continuation of this plan.
...
### Phase 5 — Structural refactor *(do last; largest diff, zero behavior change)* — ✅ COMPLETE (2026-06-23)
